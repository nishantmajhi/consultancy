function renderTable(page = 1) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    <button class="positive_button transparent_button" title="Apply for ${job.position}" onclick="applyTo('${job.id}')">✉️</button>
    <button class="positive_button transparent_button" title="Remove ${job.position}" onclick="archiveDialog('${job.id}')">❌</button>
    </div>
    </td>
    `;
