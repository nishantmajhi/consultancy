document.addEventListener("DOMContentLoaded", async () => {
  ...
  await tryFetchingClients(5);
  if (clients.length !== 0) renderTable();
  ...
  nameInput.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
      searchArchivedClient(event.target.value.trim());
      event.target.value = "";
      renderTable();
    }
  });
  ...
  document.getElementById("search_button").addEventListener("click", () => {
    searchArchivedClient(nameInput.value.trim());
    nameInput.value = "";
    renderTable();
  });
});
...
function disableMyLink() {
  ...
  const li = anchor[0].parentElement;
