<?php
    ...
    try {
        ...
        $stmt = $db->prepare("UPDATE jobs SET visibility = 'Normal' WHERE id = :jobID");
        $stmt->bindValue(':jobID', $jobID, SQLITE3_TEXT);

        if ($stmt->execute()) {
            echo json_encode(["success" => "Job relisted successfully."]);
        } else {
            http_response_code(500);
            echo json_encode(["error" => "Failed to relist the job."]);
        }    
