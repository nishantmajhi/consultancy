function renderTable(page = 1) {
  ...
  for (let i = startIndex; i < endIndex; i++) {
    ...
    <button class="positive_button transparent_button" title="View ${
      client.name
    }" onclick="showClient('${client.id}')">👀</button>
    <button class="positive_button transparent_button" title="Remove ${
      client.name
    }" onclick="archiveDialog('${client.id}')">❌</button>
    </div>
    </td>
    `;
